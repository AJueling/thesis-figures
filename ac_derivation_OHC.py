...
class DeriveOHC(object):
    ...
    def generate_OHC_files(self, run, year=None, pwqd=False):
        ...
        assert run in ['ctrl', 'rcp', 'hq', 'lpd', 'lc1', 'lpi', 'lr1', 'lq']

        if run in ['rcp', 'hq']:
            domain = 'ocn'
        elif run=='ctrl':
            domain = 'ocn_rect'
        elif run in ['lpd', 'lpi', 'lc1', 'lr1', 'lq']:
            domain = 'ocn_low'
        ...
        def round_tlatlon(das):
            ...
            return das
        if domain=='ocn':
            HTN = round_tlatlon(HTN)
            LATS = round_tlatlon(LATS)
...
#         if os.path.isfile(file_out):  os.remove(file_out)
        combined = xr.open_mfdataset(mfname, combine='nested', concat_dim='time', coords='minimal')
...
#         OHC_integrals(run=run, mask_nr=mask_nr)
    DeriveOHC().generate_OHC_files(run=run, year=year)
    DeriveOHC().combine_yrly_OHC_integral_files(run=run)
