...
def bias_maps(do, dh, dl, kw1, kw2=None, biases=None, fn=None):
    """"""
    ...
    if biases is not None:
        for i, ax in enumerate([axh, axl]):
            ax.text(.99,0.01,biases[['H','L'][i]+'R-CESM'], fontsize=6, ha='right', transform=ax.transAxes)
        if 'OBS' in biases:
            axo.text(.99,0.01,biases['OBS'], fontsize=6, ha='right', transform=axo.transAxes)
